...
export function WeatherDetails({
  ...
  const items: { icon: IconName; label: string; value: string }[] = [
    { icon: 'cloud', label: 'Humidity', value: `${humidity}%` },
    {
      icon: 'wind',
      label: 'Wind',
      value: `${windSpeed} ${windSpeedUnit} ${getWindDirectionLabel(windDirection)}`,
    },
    ...
    <View style={styles.frame}>
      {items.map((item) => (
        <View key={item.label} style={styles.cell}>
          <PixelIcon name={item.icon} size={30} color={colors.primary} />
          <View style={styles.copy}>
            <Text style={styles.label}>{item.label.toUpperCase()}</Text>
            <Text style={styles.value}>{item.value.toUpperCase()}</Text>
          </View>
        </View>
      ))}
    </View>
