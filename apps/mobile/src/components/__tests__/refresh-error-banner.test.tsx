...
describe('RefreshErrorBanner', () => {
  it('shows the refresh error message', async () => {
    const { getByText } = await render(
      <RefreshErrorBanner message="Unable to refresh weather." onRetry={jest.fn()} />,
    );

    expect(getByText('Unable to refresh weather.')).toBeTruthy();
  });
  ...
  it('calls onRetry when Retry is pressed', async () => {
    ...
    const { getByText } = await render(
      <RefreshErrorBanner message="Unable to refresh weather." onRetry={onRetry} />,
    );
